      ...
      <div className="min-h-screen bg-cream pt-24 pb-32">
        <div className="container mx-auto px-6 max-w-7xl">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-20">
            ...
            >
              <div>
                <h1 className="text-4xl md:text-5xl font-black text-primary mb-4 tracking-tight">Gâteau Personnalisable</h1>
                <p className="text-2xl font-bold text-gray-800 tracking-wide">{basePrice} FCFA</p>
              ...
              <div className="space-y-4">
                <h3 className="text-xl font-bold text-gray-900 border-b-2 border-white pb-2">1. Type de glaçage</h3>
                <div className="flex flex-wrap gap-3">
                  {glazes.map(g => (
                    <button 
                      key={g} 
                      onClick={() => setGlaze(g)}
                      className={`px-5 py-2.5 rounded-full text-sm font-bold transition-all shadow-sm ${glaze === g ? 'bg-primary text-white scale-105' : 'bg-white text-gray-600 hover:bg-gray-50'}`}
                    >
                      {g}
                    </button>
                  ))}
              ...
              {/* Décoration */}
              <div className="space-y-4">
                <h3 className="text-xl font-bold text-gray-900 border-b-2 border-white pb-2">2. Décoration disponible</h3>
                <div className="flex flex-wrap gap-3">
                  {decos.map(d => (
                    <button 
                      key={d} 
                      onClick={() => setDeco(d)}
                      className={`px-5 py-2.5 rounded-full text-sm font-bold transition-all shadow-sm ${deco === d ? 'bg-primary text-white scale-105' : 'bg-white text-gray-600 hover:bg-gray-50'}`}
                    >
                      {d}
                    </button>
                  ))}
              ...
              {/* Couleurs de déco */}
              <div className="space-y-4">
                <h3 className="text-xl font-bold text-gray-900 border-b-2 border-white pb-2">3. Couleurs du thème</h3>
              ...
              {/* Suppléments */}
              <div className="space-y-4">
                <h3 className="text-xl font-bold text-gray-900 border-b-2 border-white pb-2">4. Suppléments (+1500 FCFA/ea)</h3>
              ...
              {/* Accessoires */}
              <div className="space-y-4">
                <h3 className="text-xl font-bold text-gray-900 border-b-2 border-white pb-2">5. Accessoires de décoration (+500 FCFA/ea)</h3>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  {accessList.map(a => (
              ...
              <div className="mt-12 bg-white rounded-2xl p-6 md:p-8 shadow-2xl border-2 border-cream-dark sticky bottom-4 z-40">
                <div className="flex justify-between items-center mb-6">
                  <span className="text-gray-500 font-bold text-lg">Total calculé :</span>
                  <span className="text-3xl font-black text-primary">{calculateTotal()} <span className="text-xl">FCFA</span></span>
                </div>
                <motion.button 
                  whileHover={{ scale: 1.02 }}
                  whileTap={{ scale: 0.98 }}
                  className="w-full bg-primary text-white py-4 rounded-xl font-black text-lg tracking-wider uppercase shadow-lg hover:bg-primary-dark transition-all cursor-pointer"
                >
                  Ajouter au panier
                </motion.button>
              </div>
